...
const FeaturesPage = () => {
  ...
  return (
    <div className="w-full flex justify-center items-center my-12 h-auto">
      <div
        className=" max-w-7xl w-[85%] px-5 flex justify-center items-center  rounded-2xl bg-cover bg-center bg-[#242424] h-auto lg:h-[250px]"
      ...
      >
        <div className="cards w-full max-lg:h-auto max-lg:flex-wrap flex gap-4 justify-center sm:justify-between items-center">
          {features_data.map(({ title, description }, index) => (
            <Card key={index} className="lg:w-[250px] sm:w-[45%] h-auto flex flex-col gap-3 bg-transparent border-none">
              <CardContent className="text-white flex flex-col gap-3 px-1">
                {index === 0 && <MdFastfood size={40} className="text-red-700" /> || index === 1 && <PiNotebookBold size={40} className="text-red-700" /> || index === 2 && <TbTruckDelivery size={40} className="text-red-700" /> || index === 3 && <GiChickenOven size={40} className="text-red-700" />}
