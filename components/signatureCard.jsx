  ...
  return (
    <>
      {window.innerWidth <= 768 ? (
        ...
        <>
          {signature_Card_data.map(({ image, title, description }, index) => (
            <div
              key={index}
              className="popular-card w-[300px] h-[400px] rounded-lg relative font-['Jost'] bg-cover bg-center bg-[#008141]"
              style={{ backgroundImage: `url(${PopularBgImage.src})` }}
            >
              {/* Image Container */}
              <div className="w-full h-full p-3 flex flex-col gap-5 items-center">
                ...
                {/* Content */}
                <div className="flex flex-col justify-center gap-2 items-center w-full h-auto">
                  <h3 className="text-xl font-bold bg-white zilla uppercase text-[#008141]">
                    {title}
                  </h3>
                  <p className="text-sm text-center text-white urbanist">
                    {description}
